...
export default function LawyerLayout() {
  ...
  useEffect(() => {
    ...
    ServiceRequest.filter({ status: "submitted" }, "-created_date", 50)
      .then((r) => setNewRequestsCount(r.length))
      .catch(() => {});
  ...
  return (
    ...
    >
      <div
        className="fixed top-0 left-0 right-0 z-40 bg-white border-b"
        style={{
          paddingTop: "env(safe-area-inset-top, 0px)",
          borderColor: "#E7ECF3",
          boxShadow: "0 1px 8px rgba(18,62,124,0.06)",
        }}
      >
        <div className="flex items-center justify-between px-4" style={{ height: "52px" }}>
          ...
          <div className="flex items-center gap-1.5">
            ...
            <button
              onClick={() => navigate("/office-analytics")}
            ...
            {isAdmin && (
              ...
              >
                <Shield
                  className="w-4 h-4"
                  style={{ color: location.pathname === "/monitoring" ? "#B42318" : "#123E7C" }}
                />
              </button>
            )}
      ...
      <main
        ...
        id="lawyer-main-scroll"
        style={{ paddingTop: "calc(env(safe-area-inset-top, 0px) + 56px)", paddingBottom: "80px" }}
      >
        <Outlet />
      </main>
                ...
                {moreTabs.map((tab) => {
                  ...
                  return (
                    <button
                      ...
                      style={{
                        borderColor: isActive ? `${tab.color}40` : "#E7ECF3",
                    ...
                    >
                      ...
                      <span className="text-xs font-semibold" style={{ color: isActive ? tab.color : "#101828" }}>
                        {tab.label}
                      </span>
                    </button>
                  );
      ...
      <nav
        ...
        style={{
          ...
          boxShadow: "0 -2px 12px rgba(18,62,124,0.07)",
        }}
      >
        <div className="flex items-center h-full px-4 gap-3">
          {mainTabs.map((tab) => {
            ...
            return (
              <Link key={tab.path} to={tab.path} className="flex flex-col items-center justify-center gap-1 flex-1 h-full">
                <motion.div
                ...
                >
                  ...
                  <Icon className="w-5 h-5 relative z-10" style={{ color: isActive ? "white" : "#9CA3AF" }} />
                </motion.div>
                <span className="text-[10px] font-bold" style={{ color: isActive ? "#123E7C" : "#9CA3AF" }}>
                  {tab.label}
                </span>
              </Link>
            );
          ...
          <button onClick={() => setShowMore(true)} className="flex flex-col items-center justify-center gap-1 flex-1 h-full">
            <motion.div
            ...
            <span className="text-[10px] font-bold" style={{ color: showMore ? "#123E7C" : "#9CA3AF" }}>
              المزيد
            </span>
          </button>
        </div>
