...
import { CheckCircle2, XCircle, MapPin, User, Building2, ChevronDown, ChevronUp, Search } from "lucide-react";
...
export default function AdminPanel() {
  ...
  const loadRequests = async () => {
    ...
    try {
      ...
      const { data: allProfiles, error: statsError } = await supabase.from("profiles").select("status");
      ...
      console.error(err);
      setActionError("تعذر تحميل بيانات الإدارة");
      setRequests([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleApprove = async (id) => {
    ...
    if (error) {
      setActionError("تعذر اعتماد الطلب");
      return;
    }
    loadRequests();
  };
  ...
  const filtered = requests.filter(
    (r) => !search || r.full_name?.includes(search) || r.email?.includes(search) || r.phone?.includes(search)
  );
  ...
  if (isLoadingAuth) {
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: "linear-gradient(135deg, #0D2F5F 0%, #0a1e3d 100%)" }}>
        <div className="w-8 h-8 border-2 border-t-transparent rounded-full animate-spin" style={{ borderColor: "#C8A96B", borderTopColor: "transparent" }} />
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen" style={{ background: "linear-gradient(135deg, #0D2F5F 0%, #0a1e3d 100%)" }} dir="rtl">
      <div className="max-w-2xl mx-auto px-4 py-6">
        ...
        {actionError && (
          <div className="mb-4 rounded-xl px-3 py-2 text-sm" style={{ backgroundColor: "rgba(239,68,68,0.15)", border: "1px solid rgba(239,68,68,0.3)", color: "#fecaca", ...font }}>
            {actionError}
          </div>
        )}
        ...
        {loading ? (
          <div className="text-center py-12">
            <div className="w-8 h-8 border-2 border-t-transparent rounded-full animate-spin mx-auto" style={{ borderColor: "#C8A96B", borderTopColor: "transparent" }} />
          </div>
        ) : filtered.length === 0 ? (
          <div className="text-center py-12">
          ...
          <div className="space-y-3">
            <AnimatePresence>
              {filtered.map((req) => (
                ...
                >
                  <button onClick={() => setExpanded(expanded === req.id ? null : req.id)} className="w-full flex items-center gap-3 p-3 text-right">
                      ...
                      >
                        <div className="p-3 space-y-3">
                          <div className="grid grid-cols-2 gap-2 text-xs" style={{ color: "rgba(255,255,255,0.7)", ...font }}>
                            ...
                            <div><span style={{ color: "rgba(255,255,255,0.4)" }}>الدور: </span>{req.role === "lawyer" ? "محامي" : req.role === "admin" ? "مدير" : "عميل"}</div>
                          ...
                          {req.registration_lat && req.registration_lng ? (
                            <button
                              onClick={() => openMap(req.registration_lat, req.registration_lng)}
                            ...
                            <div className="flex items-center gap-2 px-3 py-2 rounded-lg" style={{ backgroundColor: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.2)" }}>
          ...
          {showRejectModal && (
            ...
            >
              <motion.div
                ...
                style={{ backgroundColor: "#0D2F5F", border: "1px solid rgba(255,255,255,0.15)" }}
              ...
              >
                ...
                <div className="flex gap-2 mt-3">
                  <button onClick={() => handleReject(showRejectModal)} className="flex-1 py-2.5 rounded-xl text-sm font-semibold" style={{ backgroundColor: "#ef4444", color: "white", ...font }}>
                    تأكيد الرفض
                  </button>
                  <button onClick={() => setShowRejectModal(null)} className="flex-1 py-2.5 rounded-xl text-sm font-semibold" style={{ backgroundColor: "rgba(255,255,255,0.1)", color: "white", ...font }}>
                    إلغاء
                  </button>
                </div>
              </motion.div>
